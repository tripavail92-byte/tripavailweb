...
export default function BecomePartnerPage() {
  ...
  const startFlow = async (providerType: 'HOTEL_MANAGER' | 'TOUR_OPERATOR') => {
    try {
      ...
      setError(null);
      await startProviderOnboarding({ providerType });
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 space-y-4">
        ...
        {!loading && !user && (
          <div className="rounded-lg border bg-white p-4 shadow-sm">
            ...
            <Link href="/auth/login" className="mt-2 inline-block rounded-md bg-black px-4 py-2 text-white">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-neutral-100 px-2 py-1 text-xs">Rooms & packages</span>
            </div>
            <p className="mt-2 text-sm text-neutral-700">
              Onboard your property once. Reuse rooms, amenities, and policies to publish hotel packages fast.
            </p>
            ...
            >
              {busy === 'HOTEL_MANAGER' ? 'Starting…' : 'Start Hotel Onboarding'}
            </button>
          </div>
          ...
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-neutral-100 px-2 py-1 text-xs">Tours & departures</span>
            </div>
            <p className="mt-2 text-sm text-neutral-700">
              Build 14-step tour itineraries with pickups, departures, add-ons, and compliance in one flow.
            </p>
            <ul className="mt-3 space-y-1 text-sm text-neutral-700">
            ...
            >
              {busy === 'TOUR_OPERATOR' ? 'Starting…' : 'Start Operator Onboarding'}
            </button>
          </div>
        </div>
      </main>
